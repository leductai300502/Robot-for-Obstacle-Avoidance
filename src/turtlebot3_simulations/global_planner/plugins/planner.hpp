...
geometry_msgs::Point getRandomState(costmap_2d::Costmap2DROS *costmap_ros,
  ...
  while (!pointIsFree) {
	...
	if(randValue < 0.2)
	{
		randomState.x = x_final.pose.position.x;
		randomState.y = x_final.pose.position.y;
	}
	else{
        randomState.x =
...
rapidtree generateRapidtree(geometry_msgs::PoseStamped x_init,
  ...
  for (int k = 1; k <= K; k++) {
    ...
    x_rand = getRandomState(T.X_space,x_final, robot_radius);
...
void computeMetrics(rapidtree &rrt, const geometry_msgs::PoseStamped &goal,
  ...
  computationTime = ros::Time::now().toSec() - startTime + 0.2;
