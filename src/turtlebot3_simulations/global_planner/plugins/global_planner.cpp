...
bool GlobalPlanner::makePlan(const geometry_msgs::PoseStamped &start,
  ...
  if (T_out.success) {
    ROS_INFO("IN HEHEHEHEHE");
    ROS_INFO("Start node: %.2f, %.2f ", start.pose.position.x,start.pose.position.y);
    ROS_INFO("Goal : %.2f, %.2f ", goal.pose.position.x,goal.pose.position.y); 
    ...
    ROS_INFO("Path length: %.2f m", pathLength);
    ...
    const char *path =
        "/home/catkin_ws/src/turtlebot3_simulations/global_planner/plugins/rrt_performance.txt";
